...
export function enablePersistence(
  ...
  const internalFirestore = firestore as Firestore;
  if (internalFirestore._firestoreClient) {
    throw new FirestoreError(
  ...
  const indexedDbPersistenceSettings = new IndexedDbPersistenceSettings(
    internalFirestore._settings.cacheSizeBytes,
    synchronizeTabs
  );
  ...
  internalFirestore._configureClient(persistenceWithFallback);
...
export function clearPersistence(
  ...
  const persistenceKey = IndexedDbPersistence.buildStoragePrefix(
    internalFirestore.makeDatabaseInfo()
  );
  const deferred = new Deferred<void>();
  internalFirestore._queue.enqueueAndForgetEvenAfterShutdown(async () => {
    if (
      internalFirestore._firestoreClient !== undefined &&
      !internalFirestore._firestoreClient.clientTerminated
    ) {
      deferred.reject(
        new FirestoreError(
          Code.FAILED_PRECONDITION,
